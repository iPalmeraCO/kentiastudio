...
class AddCookieNotice_Settings {
    ...
    public static function getInstance() {
        if ( !self::$_instance ) {
            self::$_instance = new AddCookieNotice_Settings();
        }
    ...
    function callback_select( $args ) {
        ...
        $html = sprintf( '<select type="number" class="number" name="%2$s[%3$s]" id="%2$s[%3$s]">', $size, $args['section'], $args['id'] );
        foreach ($args['options'] as $key => $label) {
